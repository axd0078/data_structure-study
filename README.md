...
| [programming](./programming) | 综合题目 | 已按算法思想分组的双指针、二分、图搜索、动态规划、贪心等题目，并包含补充题、CSP / GPLT 赛题归档与空 `main` 练习入口 |
| [cpp-learning](./cpp-learning) | C++ 学习路线 | 10 课 C++ 基础讲义、100 天 CSP 系统学习计划，以及配套题库与本地练习环境 |
| [summary](./summary) | 学习总结 | 指针安全、输入输出、算法模板、数据结构与刷题复盘笔记 |

## 推荐学习路径

1. `cpp-learning` 用于补齐 C++ 语法、STL 容器，并结合 `csp-100days` 推进阶段性训练
...
- `cpp-learning/README.md` 负责整理 C++ 分课讲义、100 天 CSP 学习资料与本地练习方式。
...
- 埃氏筛、匹配子序列数量、多轮筛选、安全格子统计、CSP / GPLT 赛题中的相反数对统计、窗口点击模拟、门禁系统计数输出、安全指数加权求和、灰度直方图统计、邻域均值二维前缀和、数组推导上下界计算、序列查询分段贡献累计、序列查询新解分块切段、未初始化告警统计、混合进制展开、现值折现累计、训练计划时序推导、矩形覆盖面积裁剪统计、垦田计划二分可行天数、棋盘状态重复计数、坐标平移变换、前缀缩放旋转复合、编号归属统计、相似度大小写归一后交并统计、矩阵拉平重排输出、矩阵转置查询模拟、字符串多轮映射倍增跳转、密码复杂度分类判断、边界内指令移动、分层区间推进最少步数、百分比网格定位、马步可达格子统计、72 位短消息解码、蒙特卡洛估算 pi、阈值模板检测、9 位分组三段映射变换、字符矩阵旋转与翻转恢复、咖啡时间优化、空闲区间分配与循环发送模拟、栈辅助序列构造、堆栈计算器、老板空闲时间、外卖路径统计、最佳异性缘筛选、养老院管理统计、吃火锅文本检索、刮彩票等扩展题
