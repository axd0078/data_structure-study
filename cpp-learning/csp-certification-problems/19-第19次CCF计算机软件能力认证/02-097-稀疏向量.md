...
[下载题目目录（样例文件）](../assets/staticdata/down/CSP202006-2.zip)
...
| 测试点 | $n$ | $a, b$ |
| --- | --- | --- |
| 1,2,3 | $\leq 10^{5}$ | $\leq 10^{3}$ |
| 4,5,6 | $\leq 5 \times 10^{5}$ | $\leq 10^{5}$ |
| 7,8,9,10 | $\leq 10^{9}$ | $\leq 5 \times 10^{5}$ |
