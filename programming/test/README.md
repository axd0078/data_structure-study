...
| `csp/10/1.cpp` | 分蛋糕题目骨架 | 当前仅保留输入输出模板 | `-` |
| `csp/11/1.cpp` | 打酱油题目骨架 | 当前仅保留输入输出模板 | `-` |
...
`csp/9/1.cpp`、`csp/10/1.cpp`、`csp/11/1.cpp` 和 `csp/12/1.cpp` 当前都只保留了基础输入输出模板，分别对应中间数、分蛋糕、打酱油和最小差值四道题的后续练习入口。
...
- `csp/9/1.cpp`、`csp/10/1.cpp`、`csp/11/1.cpp` 和 `csp/12/1.cpp` 当前都还是空的 `main` 骨架，后续补题时建议同步补上题意、算法和运行示例。
