...
| `csp/12/1.cpp` | 最小差值题目骨架 | 当前仅保留输入输出模板 | `-` |
...
`csp/9/1.cpp` 用值域为 `1..1000` 的计数桶统计每个数字出现次数，再通过 `less` 和 `greater` 的数量平衡判断中间数是否存在。

`csp/10/1.cpp` 顺序读入蛋糕重量，持续累计当前朋友拿到的总重量，达到 `k` 就立即结算到下一个朋友。

`csp/11/1.cpp` 直接按 `50 / 30 / 10` 元的优惠分组做整除和取模，等价于优先使用 `5` 瓶优惠、再处理 `3` 瓶优惠和剩余原价瓶数。

`csp/12/1.cpp` 当前仍只保留了基础输入输出模板，对应最小差值题的后续练习入口。
...
- `csp/9/1.cpp` 依赖值域较小这一条件，复盘时重点看“小于它”和“大于它”的数量如何通过计数桶线性维护。
- `csp/10/1.cpp` 重点是最后一组即使不足 `k` 也要计入答案，顺序固定后不需要排序或回退。
- `csp/11/1.cpp` 当前实现直接按金额分成 `50 / 30 / 10` 三档处理，本质上对应 `5` 瓶优惠优先的常数时间公式。
- `csp/12/1.cpp` 当前仍是空的 `main` 骨架，后续补题时建议同步补上题意、算法和运行示例。
